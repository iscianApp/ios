...
//        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 320, 216);
    ...
    if (self.minLimitDate) {
        ...
        self.minLimitDate = [self dateFromString:@"190001010000" withFormat:@"yyyyMMddHHmm"];
        minDateModel = [[UUDatePicker_DateModel alloc]initWithDate:self.minLimitDate];
    }
...
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
            nameArray = @[NSLocalizedString(@"yyyy", @"年"),NSLocalizedString(@"MM", @"月"),NSLocalizedString(@"dd", @"日"),NSLocalizedString(@"HH", @"时"),NSLocalizedString(@"mm", @"分")];
            return 5;
    }
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
            nameArray = @[NSLocalizedString(@"yyyy", @"年"),NSLocalizedString(@"MM", @"月"),NSLocalizedString(@"dd", @"日")];
            return 3;
    }
    if (self.datePickerStyle == UUDateStyle_MonthDayHourMinute){
           nameArray = @[NSLocalizedString(@"MM", @"月"),NSLocalizedString(@"dd", @"日"),NSLocalizedString(@"HH", @"时"),NSLocalizedString(@"mm", @"分")];
           return 4;
    }
    if (self.datePickerStyle == UUDateStyle_HourMinute){
        nameArray = @[NSLocalizedString(@"HH", @"时"),NSLocalizedString(@"mm", @"分")];
        return 2;
    }
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    switch (self.datePickerStyle) {
        case UUDateStyle_YearMonthDayHourMinute:{
            if (component==0) return (70.0/320.0)*SCREENWIDTH;
            if (component==1) return (50.0/320.0)*SCREENWIDTH;
            if (component==2) return (50.0/320.0)*SCREENWIDTH;
            if (component==3) return (50.0/320.0)*SCREENWIDTH;
            if (component==4) return (50.0/320.0)*SCREENWIDTH;
        }
            break;
        case UUDateStyle_YearMonthDay:{
            if (component==0) return (70.0/320.0)*SCREENWIDTH;
            if (component==1) return (100.0/320.0)*SCREENWIDTH;
            if (component==2) return (50.0/320.0)*SCREENWIDTH;
        }
            break;
        case UUDateStyle_MonthDayHourMinute:{
            if (component==0) return (70.0/320.0)*SCREENWIDTH;
            if (component==1) return (60.0/320.0)*SCREENWIDTH;
            if (component==2) return (60.0/320.0)*SCREENWIDTH;
            if (component==3) return (60.0/320.0)*SCREENWIDTH;
        }
            break;
        case UUDateStyle_HourMinute:{
            if (component==0) return (100.0/320.0)*SCREENWIDTH;
            if (component==1) return (100.0/320.0)*SCREENWIDTH;
        }
            break;
        default:
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    customLabel.text = [NSString stringWithFormat:@"%@%@",title,nameArray[component]];
