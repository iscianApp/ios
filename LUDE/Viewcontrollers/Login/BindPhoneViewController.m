...
@interface BindPhoneViewController ()<UIScrollViewDelegate,UITextFieldDelegate>
...
@property (strong, nonatomic) IBOutlet UILabel *lblPerfectInfo;
//记录验证码
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [_lblPerfectInfo setText:NSLocalizedString(@"Perfect Information", nil)];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self.register_UIButton makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(self.codeView.bottom).with.offset(50.0);
            ...
            [apis POSTCheckVerificationCodeTelephone:_phoneText_UITextField.text verificationCode:_codeText_UITextField.text sendTime:_sendTimeStr andCompletion:^(id objectRet, NSError *errorRes)
             {
                 ...
                 else
                 {
                     ...
                     if ([statusStr isEqualToString:@"1"])
                     {
                         ...
                         editinfo.thirdPartUID = __weakObject.thirdPartUID;
...
-(void)Countdown
{
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(self.timeout<=0){ //倒计时结束，关闭
            ...
            NSString *strTime = [NSString stringWithFormat:@"%.2ld",(long)self.timeout];
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
