...
@interface SecondViewcontroller ()<UIScrollViewDelegate,ChartDataSource,UINavigationControllerDelegate>
{
     NSString *yearStr;
     NSString *typeStr;
...
@property (strong, nonatomic) IBOutlet UILabel *lblTitleHistory;
...
- (void)viewDidLoad
{
    ...
    [self MJView];
    [_lblTitleHistory setText:NSLocalizedString(@"History", nil)];
...
-(void)createdataTwo
{
    ...
    [apis GetBloodPressureHistoryTendencyChartRequestWithUserId:userId type:typeStr andCompletion:^(id objectRet, NSError *errorRes)
    {
        if(errorRes)
        {
...
-(void)createData
{
    ...
    [apis GetBloodPressureHistoryRequestWithUserId:userId type:typeStr pageNo:@"" pageSize:@"" andCompletion:^(id objectRet, NSError *errorRes) {
        if (errorRes)
        {
            switch (errorRes.code) {
                case -1004:
...
{
    NSString *bloodPressureId = [[[[_secondMarr objectAtIndex:indexPath.section]objectForKey:@"bloodPressureList"]objectAtIndex:indexPath.row]objectForKey:@"bloodPressureId"];
